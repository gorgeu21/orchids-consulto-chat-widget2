  ...
  return (
    <>
      ...
      {!isOpen && (
        <button
          onClick={toggleOpen}
          className={`fixed bottom-6 right-6 z-50 w-16 h-16 rounded-full shadow-2xl ${isDark ? 'bg-[#00A8E8]' : 'bg-[#10B981]'} hover:scale-110 transition-all duration-300 flex items-center justify-center group animate-fab-bounce-in`}
          aria-label="Открыть виджет консультанта"
        >
          <div className={`absolute inset-0 rounded-full ${isDark ? 'bg-[#00A8E8]' : 'bg-[#10B981]'} pulse-ring`}></div>
      ...
      {isOpen && (
        <>
          ...
          >
            <button 
              onClick={handleClose}
              className={`absolute top-4 right-4 z-10 p-2 rounded-full ${isDark ? 'bg-[#2D3F52]/50 hover:bg-[#2D3F52]' : 'bg-gray-200 hover:bg-gray-300'} ${textSecondary} hover:${textPrimary} transition-all duration-200`}
              ...
              <div className="mb-6">
                ...
                <div className="space-y-2">
                  ...
                  >
                    ...
                    <span className={`text-base sm:text-lg font-medium ${textPrimary} group-hover:text-[#0088CC] transition-colors`}>
                      Telegram
                    </span>
                  </a>
                  ...
                  >
                    ...
                    <span className={`text-base sm:text-lg font-medium ${textPrimary} group-hover:text-[#25D366] transition-colors`}>
                      WhatsApp
                    </span>
                  </a>
                  ...
                  >
                    <div className={`flex-shrink-0 w-10 h-10 sm:w-12 sm:h-12 rounded-full flex items-center justify-center text-white shadow-lg`} style={{ backgroundColor: accentColor }}>
                      ...
                      {(chat2deskStatus === 'ready' || chat2deskStatus === 'idle') && <MessageCircle className="w-6 h-6" />}
                    </div>
                    <div className="flex-1 text-left">
                      <span className={`text-base sm:text-lg font-medium ${textPrimary} transition-colors block`} style={{ color: chat2deskStatus === 'ready' ? accentColor : undefined }}>
                        Онлайн-чат на сайте
                      </span>
              ...
              <div>
                ...
                <div className="grid grid-cols-2 gap-2 sm:gap-3">
                  <a 
                    ...
                    className={`group flex flex-col items-center justify-center p-3 sm:p-4 rounded-xl ${isDark ? 'bg-[#1F2E3F] hover:bg-[#2A3B50]' : 'bg-gray-100 hover:bg-gray-200'} border ${borderColor} text-center transition-all duration-200`}
                  >
                    <Phone className="w-5 h-5 mb-2 group-hover:scale-110 transition-transform" style={{ color: accentColor }} />
                    <span className={`text-sm sm:text-base font-semibold ${textPrimary} mb-1`}>
                      8 800 600 43 22
                    </span>
...
export default function ConsultantWidget({ theme = 'dark' }: ConsultantWidgetProps) {
  ...
  const [currentTheme, setCurrentTheme] = useState<Theme>(theme);
    ...
    useEffect(() => {
      ...
      // Важно: виджет Chat2Desk должен быть скрыт всегда,
      // и появляться только после явного клика по кнопке "Онлайн-чат".
      hideChat2DeskWidget();
...
export default function ConsultantWidget({ theme = 'dark' }: ConsultantWidgetProps) {
  ...
  const initChat2Desk = () => {
    ...
    const handleOnlineChatClick = () => {
      ...
      // idle или error → грузим скрипт и откроем автоматически, когда будет готов
      setOpenChatOnReady(true);
      initChat2Desk();
    };
